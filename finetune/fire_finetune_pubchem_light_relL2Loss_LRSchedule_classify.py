...
class LightningModule(pl.LightningModule):
    ...
    def __init__(self, config, tokenizer):
        ...
        self.fcs = []
        # self.loss = torch.nn.L1Loss()  # mean absolute error
        # self.loss = LpLoss()
    ...
    def configure_optimizers(self):
        # print("******************** LightningModule.configure_optimizers() ********************")
        ...
        step_size, gamma = 50, 0.7
        # wd = 1e-4

        # optimizer = optimizers.FusedLAMB(optim_groups, lr=learning_rate, betas=betas, weight_decay=wd)
        optimizer = optimizers.FusedLAMB(optim_groups, lr=learning_rate, betas=betas)
        # lr_scheduler = scheduler(optimizer, LR_schedule(learning_rate, self.current_epoch, step_size, gamma))
...
def main():
    ...
    # checkpoint_callback = pl.callbacks.ModelCheckpoint(period=1, save_last=True, dirpath=checkpoint_dir,
    #                                                    filename='checkpoint', verbose=True)
    val_monitor = margs.measure_name + '_valid_loss'
    checkpoint_callback = pl.callbacks.ModelCheckpoint(monitor=val_monitor,
                                                       period=1,
                                                       save_last=True,
                                                       dirpath=checkpoint_dir,
                                                       filename='checkpoint-{epoch}-{' + margs.measure_name + '_valid_loss:.3f}',
                                                       verbose=True)
    # checkpoint_callback = pl.callbacks.ModelCheckpoint(monitor=margs.measure_name + '_valid_r2',
    #                                                    period=1,
    #                                                    save_last=True,
    #                                                    dirpath=checkpoint_dir,
    ...
    if margs.seed_path == '':
        ...
        print(f"# loaded pre-trained model from args.seed_path: {margs.seed_path}")
